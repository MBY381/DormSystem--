...
public class RepairMail {
    ...
    public String[] getRecipients() {
        try {
            ...
            String sql = "select u_email from user , repair where user.u_id=repair.u_id and repair.time_actual is null and repair.time_finish <= CURDATE()+1";//查询语句
    ...
    public void sendGroupMail() throws AddressException, MessagingException {
        ...
        if (recipients != null) {
            for (int i = 0; i < recipients.length; i++) {
                ...
                //设置标题
                message.setSubject("维修提醒");
                //设置正文
                message.setContent("您有维修业务尚未完成，请尽快处理", "text/html;charset=utf-8");
                //发送
