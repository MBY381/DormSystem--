      ...
      <a-layout style="padding: 0 24px 24px">
        <a-breadcrumb style="margin: 16px 0">
          ...
          <a-breadcrumb-item @click="student">学生页面</a-breadcrumb-item>
           <a-breadcrumb-item>申请换宿</a-breadcrumb-item>
        </a-breadcrumb>
<a-layout-content
...
<a-table
  ...
  >
    ...
    <template #action="{ record }">
      <span>
        <a @click="insertInfo(record.dorm_id)">申请该宿舍</a>
   ...
   <a-modal v-model:visible="visible" title="投诉" @ok="execAdd">
    ...
    >
      <a-form-item label="投诉内容">
        <a-input v-model:value="ad.tousu" />
      </a-form-item>
    </a-form>
  </a-modal>
        </a-layout-content>
...
const getPage = (pageNum = 1) => {
  ...
  get("securitystu/getInfo", query).then((res) => {
...
//点击申请该行宿舍
const insertInfo = (dorm_id)=>{
    // put('securitystu/complete'+dorm_id).then(res=>{
    //     tip.success("申请成功，等待审核");
    //     getPage(page.value.current);
    // });

    dorm.dorm_id=dorm_id;
    post("securitystu/complete",dorm).then((res)=>{
      tip.success("申请成功，等待楼管审核");
    });
};
