<template>
    ...
    <a-layout>
      <a-layout-sider width="200" style="background: #fff">
        ...
        <a-sub-menu key="sub3">
            ...
            <a-menu-item key="9" @click="userinfo4">个人信息</a-menu-item>
          ...
          <a-sub-menu key="sub1">
            ...
            <a-menu-item key="1" @click="allup()" >一键处理</a-menu-item>
      ...
      <a-layout style="padding: 0 24px 24px">
        <a-breadcrumb style="margin: 16px 0">
          ...
          <a-breadcrumb-item>维修工页面</a-breadcrumb-item>
  ...
  >
    <template #cusid="{ text }"> 编号：{{ text }} </template>
     <template #repair_status="{text}">
    {{Status[text]}}
    </template>
    <template #opr="{ record }">
     <a-button @click="addwater(record.repair_id)">维修</a-button>
      <a-divider type="vertical" />
      <a-button type="danger" @click="execDel(record.repair_id)"  >删除</a-button>
...
//更新
const allup=()=>{
  put('/repairer').then(res=>{
    tip.success("维修成功");
    getpage(page.value.current);
  });
};
const execDel=(repair_id)=>{
del('/repairer/'+repair_id).then(res=>{
        tip.success("删除成功！");
        getPage(page.value.current);
    });
};
 const addwater=(repair_id)=>{
put('/repairer/'+repair_id).then(res=>{
        tip.success("维修成功！");
        getPage(page.value.current);
    });
};
...
const userinfo4= ()=>{
    router.push({path:'/messageinfo/userinfo4'});
}
